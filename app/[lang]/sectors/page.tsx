...
export async function generateMetadata({
  params,
}: PageProps<"/[lang]/sectors">): Promise<Metadata> {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();

  return buildPageMetadata(lang, "sectors");
}

export default async function SectorsPage({ params }: PageProps<"/[lang]/sectors">) {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();

  const dict = await getSiteDictionary(lang);
  const page = dict.meta.pages.sectors;
  ...
  return (
    <div className="pt-24 sm:pt-28">
      ...
      />
      {/*
        `<Sectors />` leads with an `<h2>`, which is correct on the landing page
