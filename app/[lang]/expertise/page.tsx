...
export async function generateMetadata({
  params,
}: PageProps<"/[lang]/expertise">): Promise<Metadata> {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();

  return buildPageMetadata(lang, "expertise");
}
...
export default async function ExpertisePage({ params }: PageProps<"/[lang]/expertise">) {
  ...
  const page = dict.meta.pages.expertise;
  ...
  return (
    <div className="pt-24 sm:pt-28">
      ...
      />
      {/*
        `<Expertise />` leads with an `<h2>`, which is correct on the landing page
